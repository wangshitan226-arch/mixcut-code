...
def extract_keywords_for_pexels(sentences: List[Dict], max_keywords: int = 3) -> List[Dict]:
    """
    使用 DeepSeek 从句子中提取关键词（专用于 Pexels 搜索）
    """
    try:
        # 从已有的 kaipai_asr 导入 DeepSeek 配置
        ...
        sentences_text = "\n".join([
            f"ID:{s['id']}|{s['beginTime']}-{s['endTime']}|{s['text']}"
            for s in speech_sentences[:10]
        ])
        
        prompt = f"""从以下视频句子中提取{max_keywords}个最具视觉表现力的关键词。

句子格式：ID|开始时间-结束时间|内容
{sentences_text}

要求：
1. 关键词要具象化，能在Pexels视频素材库找到匹配素材
2. 关键词应该是名词，如"森林"、"城市"、"海浪"、"日落"
3. 返回JSON格式：[{{"keyword": "xxx", "sentence_id": "xxx"}}, ...]
4. 只返回JSON，不要其他解释"""
        ...
        if json_match:
            ...
            sentence_map = {s['id']: s for s in speech_sentences}
            result_keywords = []
            
            for kw in keywords_data[:max_keywords]:
                sentence_id = kw.get('sentence_id')
                if sentence_id in sentence_map:
                    sent = sentence_map[sentence_id]
                    result_keywords.append({
                        'keyword': kw['keyword'],
                        'sentence_id': sentence_id,
                        'beginTime': sent['beginTime'],
                        'endTime': sent['endTime']
                    })
            
            logger.info(f"[Pexels] DeepSeek 提取关键词: {[k['keyword'] for k in result_keywords]}")
            return result_keywords
        
        return []
        
    except Exception as e:
        logger.error(f"[Pexels] DeepSeek 提取关键词失败: {e}")
        return []


def generate_aliyun_subtitle_clips(
    sentences: List[Dict],
    styles_config: Dict,
    keep_segments: List[Tuple[int, int]]
) -> List[Dict]:
    """
    生成阿里云格式的字幕clips（带时间戳重映射）
    
    使用阿里云ICE字幕格式（字段名大写）：
    - FontSize, Font, FontColor, FontColorOpacity
    - Outline, OutlineColour
    - Alignment, X, Y
    - EffectColorStyle, FontFace
    """
    clips = []
    ...
    speech_sentences = [
        ...
        if s.get('type') != 'silence' and s.get('text', '').strip() and not s.get('text', '').startswith('[无声')
    ]
    
    # 获取样式配置（小写转大写）
    title_style = styles_config.get('title', {})
    subtitle_style = styles_config.get('subtitle', {})
    emphasis_style = styles_config.get('emphasis', {})
    ...
    for sent_id, mapped in time_mapping.items():
        ...
        # 根据文本长度选择样式
        if text_len <= 10 and ('！' in text or '!' in text):
        ...
        clip = {
            ...
            "X": style.get('x', 0),
            "Y": style.get('y', 900),
            "Alignment": style.get('alignment', 'TopCenter'),
            "Font": style.get('font', 'AlibabaPuHuiTi'),
            "FontSize": style.get('font_size', 60),
            "FontColor": style.get('font_color', '#FFFFFF'),
            "FontColorOpacity": style.get('font_color_opacity', 1),
        ...
        # 可选参数
        if 'effect_color_style' in style:
            clip["EffectColorStyle"] = style['effect_color_style']
        
        if 'angle' in style:
            clip["Angle"] = style['angle']
        
        if 'font_face' in style:
            font_face = style['font_face']
            clip["FontFace"] = {
                "Bold": font_face.get('Bold', False),
                "Italic": font_face.get('Italic', False),
                "Underline": font_face.get('Underline', False)
            }
        
        clips.append(clip)
    
    logger.info(f"[Aliyun字幕] 生成 {len(clips)} 个字幕clips")
    return clips


def generate_pexels_timeline(
    video_url: str,
    sentences: List[Dict],
    removed_segment_ids: List[str],
    template_config: Dict,
    ...
    logger.info(f"[Pexels] 原始片段 {len(sentences)} 个，保留 {len(kept_sentences)} 个")
    
    subtitle_styles = template_config.get('subtitleStyles', SUBTITLE_STYLES)
    pexels_config = template_config.get('pexelsConfig', {})
    blur_radius = pexels_config.get('blurRadius', 0.1)
    ...
    actual_duration_sec = timeline_position
    logger.info(f"[Pexels] 主视频生成完成，时长: {actual_duration_sec}s")
    
    # Pexels 画中画处理
    pexels_video_clips = []
    
    if user_id:
        logger.info("[Pexels] 开始提取关键词...")
        pexels_keywords = extract_keywords_for_pexels(kept_sentences, max_keywords=3)
        
        if pexels_keywords:
            logger.info(f"[Pexels] 提取到关键词: {[k['keyword'] for k in pexels_keywords]}")
            
            from utils.pexels_client import get_videos_for_keywords
            pexels_videos = get_videos_for_keywords(pexels_keywords, user_id)
            
            for pv in pexels_videos:
                begin_sec = pv['beginTime'] / 1000.0
                end_sec = pv['endTime'] / 1000.0
                begin_sec = max(0, min(begin_sec, actual_duration_sec))
                end_sec = max(0, min(end_sec, actual_duration_sec))
                ...
                if end_sec > begin_sec:
                    ...
                    pexels_video_clips.append(pexels_clip)
                    logger.info(f"[Pexels] 添加画中画: {pv['keyword']} ({begin_sec:.2f}s - {end_sec:.2f}s)")
    
    # 生成字幕 clips（阿里云格式）
    subtitle_clips = generate_aliyun_subtitle_clips(
        kept_sentences,
        subtitle_styles,
        keep_segments
    )
    ...
    audio_tracks = generate_audio_tracks_with_crop(
        video_url,
        template_config.get('backgroundMusic'),
        template_config.get('soundEffects', []),
        actual_duration_sec,
        kept_sentences,
        keep_segments
    )
    ...
    # 组装 Timeline
    video_tracks = [{"VideoTrackClips": video_clips}]
    
    if pexels_video_clips:
        video_tracks.append({"VideoTrackClips": pexels_video_clips})
    ...
    timeline = {
        ...
        "AudioTracks": audio_tracks,
        "VideoSummary": {
            "Title": asr_result.get('metadata', {}).get('title', 'Pexels画中画视频') if asr_result else "Pexels画中画视频",
            "Description": f"包含 {len(subtitle_clips)} 个字幕，{len(pexels_video_clips)} 个Pexels画中画",
            "Duration": actual_duration_sec,
            "SubtitleCount": len(subtitle_clips),
            "HasPexelsPIP": len(pexels_video_clips) > 0,
            "PexelsVideoCount": len(pexels_video_clips)
        }
    }
