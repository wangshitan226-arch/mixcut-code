...
export default function KaipaiEditor({
  ...
  // 新增：流程控制状态
  const [currentScreen, setCurrentScreen] = useState<'editor' | 'saveSuccess' | 'publishCopy'>('editor');
  ...
  // 保存编辑 - 下载视频到本地并跳转到保存成功页
  const handleSaveAndGenerateCopy = async () => {
    if (segments.length === 0) {
      alert('没有可导出的内容');
      return;
    }
    ...
    setIsExporting(true);
    setLoadingText('正在保存视频到本地...');

    try {
      // 1. 获取草稿详情
      const draftResponse = await fetch(`${API_BASE_URL}/api/kaipai/${editId}`);
      const draftData = await draftResponse.json();

      // 2. 准备数据
      const asrResult = draftData.asr_result || {};
      ...
      const metadata = asrResult.metadata || {};

      // 3. 判断使用哪个视频 URL
      const videoUrlToUse = draftData.output_video_url || 
                           draftData.output_url || 
                           originalVideoUrl || 
                           videoUrl;

      // 4. 下载视频到本地
      setLoadingText('正在下载视频...');
      const videoResponse = await fetch(videoUrlToUse);
      const videoBlob = await videoResponse.blob();
      
      // 5. 保存到本地文件系统
      const filename = `mixcut_${editId.slice(0, 8)}_${Date.now()}.mp4`;
  ...
  // 处理从保存成功页跳转到发布文案页
  const handleGoToPublishCopy = (data: {
    editId: string;
    videoText: string;
    extractedTitle: string;
    extractedKeywords: string[];
  ...
  // 根据当前屏幕返回不同的组件
  if (currentScreen === 'saveSuccess' && saveSuccessData) {
    return (
      <SaveSuccessScreen
        editId={editId}
        videoUrl={saveSuccessData.outputVideoUrl}
        onBack={handleSaveSuccessBack}
        onGenerateCopy={handleGoToPublishCopy}
      />
    );
  }

  if (currentScreen === 'publishCopy' && saveSuccessData) {
    return (
      <PublishCopyScreen
        editId={editId}
        videoText={saveSuccessData.videoText}
    ...
    <>
      ...
      <header className="flex items-center justify-between px-4 h-14 bg-white shrink-0 border-b border-gray-100">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleSaveAndGenerateCopy}
            disabled={isExporting || segments.length === 0}
            className={`text-white text-sm px-4 py-1.5 rounded-full font-bold flex items-center gap-1.5 ${
              isExporting || segments.length === 0
                ? 'bg-gray-400'
                : 'bg-blue-600'
            }`}
          >
